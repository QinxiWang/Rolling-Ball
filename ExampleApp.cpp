...
namespace basicgraphics {
    ...
    void ExampleApp::onSimulation(double rdt){
        ...
        if(dir != vec3(0)){
        vec3 axisRotate = cross(vec3(dir.x, 1, dir.z), dir);
        sphereFrame = rotate(sphereFrame, radians(5.0f), axisRotate);
        }
        sphereFrame = column(sphereFrame, 3, column(sphereFrame, 3) + vec4(dir, 1.0f));
