 ...
 */
	...
	std::shared_ptr<Texture> Texture::create2DTextureFromFile(const std::string &filename, bool generateMipMaps/*=false*/, int numMipMapLevels/*=1*/)
	{
		...
		const void* bytesArray[6];
		bytesArray[0] = image;
		std::shared_ptr<Texture> tex(new Texture(filename, width, height, 1, numMipMapLevels, generateMipMaps, GL_TEXTURE_2D, internalFormat, getExternalFormat(internalFormat), GL_UNSIGNED_BYTE, bytesArray));//internalFormat, getExternalFormat(internalFormat), determineDataType(internalFormat), bytesArray));
